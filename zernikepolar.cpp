...
double zernikePolar::zernike(size_t n){
    ...
    {
        // zernikePolar only supports up to 49 terms (order 12). Higher terms are not
        // available here; return 0 instead of throwing so that an accidental request
        // for a higher-order term degrades gracefully rather than crashing the app.
        // Surface generation for order > 12 goes through zpmC (see zernikeEditDlg).
        static bool warned = false;
        if (!warned) {
            warned = true;
            spdlog::get("logger")->warn("zernikePolar(): term {} exceeds max computed ({}); returning 0", n, m_nbTermsComputed);
        }
        return 0.;
    }
}