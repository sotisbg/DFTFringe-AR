...
void zernikeProcess::fillVoid(wavefront &wf){
    ...
    if (wf.regions.size() > 0){
        ...
        if (useannular){
            ...
            // now that we have the points in rho and theta get the zernike terms at each of those points
            arma::mat zerns = zapm( r.as_col(), t.as_col(), md->annularObsPercentClear(), 12);
...
arma::mat zernikeProcess::rhotheta( int width, double radius, double cx, double cy,
    ...
    mirrorDlg *md = mirrorDlg::get_Instance();
    if (md->m_useAnnular){
        centerR = md->annularObsPercentClear();
    }
    if (wf != 0){
        useMask = true;
...
void zernikeProcess::initGrid(int width, double radius, double cx, double cy, int maxOrder,
    ...
    mirrorDlg *md = mirrorDlg::get_Instance();
    if (md->m_useAnnular){
        obsPercent = md->annularObsPercentClear();
...
std::vector<double>  zernikeProcess::ZernFitWavefront(wavefront &wf){
    // Always rebuild the grid for the wavefront being fitted. initGrid's cache key
    // only looks at width/radius/maxOrder/obs/annulus - NOT the wavefront center or
    // its mask/data - so two different wavefronts with the same size and radius would
    // otherwise reuse stale m_rhoTheta/m_row/m_col/m_zerns and produce wrong (and
    // seemingly random) Zernike terms. Fitting is not on a hot path, so recomputing
    // the grid every time is safe and cheap.
    m_needsInit = true;
