...
/* m_annularObsPercent is the central hole as a fraction of the FULL diameter —
 * that is how the user enters it and how the outline is drawn on the igram.
 *
 * The annular Zernike basis, however, lives on the analysed wavefront, whose
 * rho is normalised to the CLEAR aperture (rhotheta() takes its radius from
 * wf->m_outside.m_radius, and dftarea cuts the analysis down to the clear
 * aperture).  With Reduce aperture on, the two are not the same:
 *
 *     hole 148 mm, blank 363 mm, reduction 6.5 mm  ->  clear 350 mm
 *     full-diameter fraction  148/363 = 0.4077
 *     clear-aperture fraction 148/350 = 0.4229
 *
 * Fitting data that spans [0.4229, 1] against polynomials orthogonal over
 * [0.4077, 1] leaves the coefficients systematically off — measured at about
 * 3% on the zonal corrections, which is enough to read as "past 100%" when
 * the surface is really at 97%.
 */
double mirrorDlg::annularObsPercentClear() const {
    if (!m_useAnnular)
        return 0.;
    double clear = m_aperatureReductionEnabled ? (diameter - aperatureReduction * 2.)
                                               : diameter;
    if (clear <= 0. || diameter <= 0.)
        return m_annularObsPercent;
    double pct = m_annularObsPercent * diameter / clear;
    return (pct >= .95) ? .95 : pct;   // keep the basis well defined
}
...
//Z = d^6 / (16 * R^5)
    ...
    if (m_useAnnular){
        double obs = annularObsPercentClear();
        double f = (1 - (obs * obs));
        f *= f;
        z8 *= f;
    }
